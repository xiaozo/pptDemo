...
- (BOOL)isNumberAllowSpace:(BOOL)allowSpace {
    NSString *match = @"(^[0-9]+$)";
    if (allowSpace) {
        match = @"(^[0-9\\s]+$)";
    }
...
- (BOOL)isChineseAllowSpace:(BOOL)allowSpace {
    NSString *match = @"(^[\u4e00-\u9fa5]+$)";
    if (allowSpace) {
        match = @"(^[\u4e00-\u9fa5\\s]+$)";
    }
...
- (BOOL)isEnglishAllowSpace: (BOOL) allowSpace {
...
///替换除了参数 其他都替换的url
///如@"http://www.baidu.com/fff/fff?a=4&b=4" 替换成@"http://www.f/fff/aaa?a=4&b=4";
- (NSString *)replaceUrlDomainPathsByNewDomainPaths:(NSString *)newDomainPaths {
    NSString *searchStr = self;
    NSString * regExpStr = @"(^[\\S\\s^\\?]*)(\\?)([\\S\\s^\\?]*)|(^[\\S\\s^\\?]*)";
    NSString * replacement = [NSString stringWithFormat:@"%@$2$3",newDomainPaths];
...
///替换域名
- (NSString *)replaceUrlDomainByNewDomain:(NSString *)newDomain {
    NSString *searchStr = self;
    NSString * regExpStr = @"(https:\\/\\/|http:\\/\\/)([^\\/]*)(\\/.*)";
    NSString * replacement = [NSString stringWithFormat:@"%@$3",newDomain];
